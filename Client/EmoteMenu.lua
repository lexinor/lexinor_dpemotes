...
mainMenu = NativeUI.CreateMenu("New Dawn", "", menuPosition["x"], menuPosition["y"], Menuthing, Menuthing)
...
function AddEmoteMenu(menu)
    ...
    local gestesmenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['gesteemotes'], "", "", Menuthing, Menuthing)
    table.insert(EmoteTable, Config.Languages[lang]['gesteemotes'])
     
    local autresmenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['autreemotes'], "", "", Menuthing, Menuthing)
    table.insert(EmoteTable, Config.Languages[lang]['autreemotes'])

    local activitesmenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['activiteemotes'], "", "", Menuthing, Menuthing)
    table.insert(EmoteTable, Config.Languages[lang]['activiteemotes'])

    local positionsmenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['positionemotes'], "", "", Menuthing, Menuthing)
    table.insert(EmoteTable, Config.Languages[lang]['positionemotes'])

    local sportsmenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['sportemotes'], "", "", Menuthing, Menuthing)
    table.insert(EmoteTable, Config.Languages[lang]['sportemotes'])

    local santesmenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['santeemotes'], "", "", Menuthing, Menuthing)
    table.insert(EmoteTable, Config.Languages[lang]['santeemotes'])
    ...
    for a,b in pairsByKeys(DP.Gestes) do
      x,y,z = table.unpack(b)
      gestesitem = NativeUI.CreateItem(z, "/e ("..a..")")
      sharegestesitem = NativeUI.CreateItem(z, "")
      gestesmenu:AddItem(gestesitem)
      if Config.GestesEmotesEnabled then
        sharegestesmenu:AddItem(sharegestesitem)
      end
      table.insert(GestesTable, a)
    end

    for a,b in pairsByKeys(DP.Autres) do
      x,y,z = table.unpack(b)
      autresitem = NativeUI.CreateItem(z, "/e ("..a..")")
      shareautresitem = NativeUI.CreateItem(z, "")
      autresmenu:AddItem(autresitem)
      if Config.AutresEmotesEnabled then
        shareautresmenu:AddItem(shareautresitem)
      end
      table.insert(AutresTable, a)
    end

    for a,b in pairsByKeys(DP.Activites) do
      x,y,z = table.unpack(b)
      activitesitem = NativeUI.CreateItem(z, "/e ("..a..")")
      shareactivitesitem = NativeUI.CreateItem(z, "")
      activitesmenu:AddItem(activitesitem)
      if Config.ActivitesEmotesEnabled then
        shareactivitesmenu:AddItem(shareactivitesitem)
      end
      table.insert(ActivitesTable, a)
    end

    for a,b in pairsByKeys(DP.Positions) do
      x,y,z = table.unpack(b)
      positionsitem = NativeUI.CreateItem(z, "/e ("..a..")")
      sharepositionsitem = NativeUI.CreateItem(z, "")
      positionsmenu:AddItem(positionsitem)
      if Config.PositionsEmotesEnabled then
        sharepositionsmenu:AddItem(sharepositionsitem)
      end
      table.insert(PositionsTable, a)
    end

    for a,b in pairsByKeys(DP.Sports) do
      x,y,z = table.unpack(b)
      sportsitem = NativeUI.CreateItem(z, "/e ("..a..")")
      sharesportsitem = NativeUI.CreateItem(z, "")
      sportsmenu:AddItem(sportsitem)
      if Config.SportsEmotesEnabled then
        sharesportsmenu:AddItem(sharesportsitem)
      end
      table.insert(SportsTable, a)
    end

    for a,b in pairsByKeys(DP.Santes) do
      x,y,z = table.unpack(b)
      santesitem = NativeUI.CreateItem(z, "/e ("..a..")")
      sharesantesitem = NativeUI.CreateItem(z, "")
      santesmenu:AddItem(santesitem)
      if Config.SantesEmotesEnabled then
        sharesantesmenu:AddItem(sharesantesitem)
      end
      table.insert(SantesTable, a)
    end
    ...
    gestesmenu.OnItemSelect = function(sender, item, index)
      EmoteMenuStart(GestesTable[index], "gestes")
    end
    ...
    sportsmenu.OnItemSelect = function(sender, item, index)
      EmoteMenuStart(SportsTable[index], "sports")
    end

    santesmenu.OnItemSelect = function(sender, item, index)
      EmoteMenuStart(SantesTable[index], "santes")
    end
