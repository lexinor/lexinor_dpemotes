...
Citizen.CreateThread(function()
  while true do
    ...
    if Config.EnableXtoCancel then 
      if IsControlPressed(1, 73) or IsControlJustReleased(1, 105) then 
        EmoteCancel() 
      end 
    end
    Citizen.Wait(1)
...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/e', "Jouer une anim'", {{ name="emotename", help="dance, camera, sit or any valid emote."}})
    TriggerEvent('chat:addSuggestion', '/e', "Jouer une anim'", {{ name="emotename", help="dance, camera, sit or any valid emote."}})
    TriggerEvent('chat:addSuggestion', '/emote', "Jouer une anim'", {{ name="emotename", help="dance, camera, sit or any valid emote."}})
    if Config.SqlKeybinding then
      TriggerEvent('chat:addSuggestion', '/emotebind', "Bind une anim'", {{ name="key", help="num4, num5, num6, num7. num8, num9. Numpad 4-9!"}, { name="emotename", help="dance, camera, sit or any valid emote."}})
      TriggerEvent('chat:addSuggestion', '/emotebinds', "Lister les anim' bind")
    end
...
function EmoteMenuStart(args, hard)
    ...
    elseif etype == "gestes" then
        if DP.Gestes[name] ~= nil then
          if OnEmotePlay(DP.Gestes[name]) then end
        end
    elseif etype == "autres" then
        if DP.Autres[name] ~= nil then
          if OnEmotePlay(DP.Autres[name]) then end
        end
    elseif etype == "activites" then
        if DP.Activites[name] ~= nil then
          if OnEmotePlay(DP.Activites[name]) then end
        end
    elseif etype == "positions" then
        if DP.Positions[name] ~= nil then
          if OnEmotePlay(DP.Positions[name]) then end
        end
    elseif etype == "sports" then
        if DP.Sports[name] ~= nil then
          if OnEmotePlay(DP.Sports[name]) then end
        end
    elseif etype == "santes" then
        if DP.Santes[name] ~= nil then
          if OnEmotePlay(DP.Santes[name]) then end
        end
...
function EmoteCommandStart(source, args, raw)
    ...
    elseif DP.Gestes[name] ~= nil then
      if OnEmotePlay(DP.Gestes[name]) then end return
    elseif DP.Autres[name] ~= nil then
      if OnEmotePlay(DP.Autres[name]) then end return
    elseif DP.Activites[name] ~= nil then
      if OnEmotePlay(DP.Activites[name]) then end return
    elseif DP.Positions[name] ~= nil then
      if OnEmotePlay(DP.Positions[name]) then end return
    elseif DP.Sports[name] ~= nil then
      if OnEmotePlay(DP.Sports[name]) then end return
    elseif DP.Santes[name] ~= nil then
      if OnEmotePlay(DP.Santes[name]) then end return
    else
...
function OnEmotePlay(EmoteName)
  ...
  TaskPlayAnim(GetPlayerPed(-1), ChosenDict, ChosenAnimation, 2.0, 2.0, AnimationDuration, MovementType, -1, false, false, false) --0 -1
